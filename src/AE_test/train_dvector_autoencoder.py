...
MODEL_SAVE_DIR = 'test_outputs/models/greedy_finetune/dvector_ae-61_100pct_75utt-50spk+300Dev_5s_100pctmainspk_100pctAmp-mainOnly_otherSingles_greedyPretrain-2000_1e-6_200ep_3'  # Directory to save trained model and config
...
def detect_speakers_from_overlap_samples(overlap_dirs):
    ...
    for overlap_dir in overlap_dirs:
        ...
        # Prefer metadata.json (ground truth per overlap sample)
        ...
        if metadata_path.exists():
            ...
            for metadata in metadata_list:
                has_main = metadata.get('has_main_speaker', None)
                main_spk = metadata.get('main_speaker', '')
                other_spks = metadata.get('other_speakers', [])
                if main_spk and (has_main is None or has_main is True):
                    all_main_speakers.add(str(main_spk))
                if isinstance(other_spks, list):
                    all_other_speakers.update(str(s) for s in other_spks if s)
                elif other_spks:
                    all_other_speakers.add(str(other_spks))

            print(f"  From {overlap_path.name}/metadata.json: Detected speakers from metadata")
            continue

        # Fallback: load speaker_config.json (if generated by new script)
...
def create_training_pairs(overlap_dvectors, clean_main_dvectors, main_speaker_ids, n_pairs_per_dir=-1):
    ...
    for key, data in overlap_dvectors.items():
        ...
        if main_spk == primary_speaker:
            ...
            if primary_speaker in other_spks:
                source_dir = data.get('source_dir', 'unknown')
                overlaps_by_dir[source_dir].append(key)
                overlap_target_speakers[key] = primary_speaker
    ...
    for source_dir, dir_overlap_keys in overlaps_by_dir.items():
        ...
        for overlap_key in selected_overlap_keys:
            ...
            main_spk = overlap_target_speakers.get(overlap_key, overlap_item.get('main_speaker', ''))
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if noise_negative_pool is not None and noise_negative_pool.numel() > 0:
                noise_idx = torch.randint(
                    0, noise_negative_pool.size(0), (overlap.size(0),), device=overlap.device
                )
                noise_target = noise_negative_pool[noise_idx]
                noise_target = F.normalize(noise_target, p=2, dim=1, eps=LOSS_COSINE_EPS)
                noise_cos = F.cosine_similarity(
                    reconstructed, noise_target, dim=1, eps=LOSS_COSINE_EPS
                )
                negative_loss = negative_loss + F.relu(noise_cos - NEGATIVE_CONTRASTIVE_MARGIN).mean()
            loss = (
...
def evaluate_model(model, test_loader, device, noise_negative_pool=None):
...
def main():
    ...
    if INCLUDE_NOISE_IDENTITY and len(noise_dvectors_dict) > 0:
        noise_items = [v['dvector'] for v in noise_dvectors_dict.values() if 'dvector' in v]
        if noise_items:
            noise_negative_pool = torch.tensor(
                np.stack(noise_items).astype(np.float32), device=DEVICE
            )
    ...
    history, best_val_loss = train_model(
        ...
        DEVICE,
        save_dir,
        noise_negative_pool=noise_negative_pool,
    )
    ...
    eval_results = evaluate_model(
        model, test_loader, DEVICE, noise_negative_pool=noise_negative_pool
    )
    ...
    config = {
        ...
        'negative_uses_noise_pool': bool(noise_negative_pool is not None),
