...
OVERLAP_SAMPLES_DIRS = ['test_outputs/data/test_other_mix_1,6s_100000_balanced_4-1','test_outputs/data/train_other_mix_1,6s_100000_balanced_babble_11-4']  # Overlap/mix dataset dirs from generate_full_overlap_utterances.py
MODEL_SAVE_DIR = 'test_outputs/models/dvector_ae_identity_1100x50Dev_wOV_balanced_1,6s_v2_12-4-26'  # Directory to save trained model and config
...
# Extra MUSAN speech-only identity pairs (input -> same input)
INCLUDE_MUSAN_SPEECH_ONLY_IDENTITY = True
N_MUSAN_SPEECH_ONLY_SAMPLES = N_NOISE_SAMPLES_PER_TYPE
MUSAN_SPEECH_ONLY_SAMPLE_DURATION = NOISE_SAMPLE_DURATION
...
# NOTE: This v2 script reuses DvectorAutoencoder from autoencoder_utils so
# training and inference architectures stay identical.
...
def extract_musan_speech_only_dvectors(
    musan_root,
    n_samples=100,
    ...
    """Extract MUSAN speech-only d-vectors for identity mapping (speech noise -> same speech noise)."""
    print(f"\n🗣️  Extracting MUSAN SPEECH-ONLY identity d-vectors...")
    print(f"  MUSAN root: {musan_root}")
    ...
    speech_dir = musan_path / 'musan_speech_train'
    if not speech_dir.exists():
        print(f"  ⚠️  MUSAN speech train directory not found: {speech_dir}")
        return results

    wav_scp = speech_dir / 'wav.scp'
    if not wav_scp.exists():
        print(f"  ⚠️  wav.scp not found in {speech_dir}")
        return results
    ...
    with open(wav_scp, 'r') as f:
        for line in f:
            ...
            if not audio_path.is_absolute():
                audio_path = musan_path.parent / audio_path

            if audio_path.exists():
                audio_files.append((utt_id, str(audio_path)))

    if not audio_files:
        print(f"  ⚠️  No valid MUSAN speech audio files found in {wav_scp}")
        return results

    print(f"  Found {len(audio_files)} MUSAN speech files")

    n_to_sample = min(n_samples, len(audio_files))
    np.random.seed(RANDOM_SEED + 200)
    sampled_indices = np.random.choice(len(audio_files), size=n_to_sample, replace=False)

    for idx in sampled_indices:
        utt_id, audio_path = audio_files[idx]

        try:
            audio, _ = librosa.load(audio_path, sr=sample_rate, duration=noise_sample_duration)
            ...
            norm = np.linalg.norm(dvector)
            if norm > 0:
                dvector = dvector / norm

            key = f"musan_speech_only_{utt_id}"
            results[key] = {
                'dvector': dvector,
                'type': 'musan_speech_only',
                'source': audio_path,
            }

        except Exception as e:
            print(f"    ⚠️  Error processing MUSAN speech file {utt_id}: {e}")
            continue

    print(f"✓ Total MUSAN speech-only identity d-vectors: {len(results)}")
    return results
...
def train_model(model, train_loader, val_loader, num_epochs, learning_rate, device, save_dir):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_input, batch_target in train_loader:
            ...
            batch_target_norm = F.normalize(batch_target, p=2, dim=1, eps=LOSS_COSINE_EPS)
            
            # Forward pass
            optimizer.zero_grad()
            reconstructed = model(batch_input)
            mse_loss = F.mse_loss(reconstructed, batch_target_norm)
            cosine_loss = 1.0 - F.cosine_similarity(
                reconstructed, batch_target_norm, dim=1, eps=LOSS_COSINE_EPS
            ).mean()
...
def evaluate_model(model, test_loader, device):
    ...
    for i in range(len(all_input)):
        denom = np.linalg.norm(all_input[i]) * np.linalg.norm(all_reconstructed[i])
        cos_sim = np.dot(all_input[i], all_reconstructed[i]) / max(denom, 1e-12)
...
def main():
    ...
    print(f"  Include MUSAN speech-only identity: {INCLUDE_MUSAN_SPEECH_ONLY_IDENTITY}")
    if INCLUDE_MUSAN_SPEECH_ONLY_IDENTITY:
        print(f"    MUSAN root: {MUSAN_ROOT}")
        print(f"    MUSAN speech-only samples: {N_MUSAN_SPEECH_ONLY_SAMPLES}")
        print(f"    MUSAN speech-only sample duration: {MUSAN_SPEECH_ONLY_SAMPLE_DURATION}s")
    ...
    # 5. Extract extra MUSAN speech-only d-vectors if enabled
    if INCLUDE_MUSAN_SPEECH_ONLY_IDENTITY:
        musan_speech_only_dvectors = extract_musan_speech_only_dvectors(
            MUSAN_ROOT,
            n_samples=N_MUSAN_SPEECH_ONLY_SAMPLES,
            noise_sample_duration=MUSAN_SPEECH_ONLY_SAMPLE_DURATION,
            sample_rate=SAMPLE_RATE,
            device=DEVICE,
        )
        all_dvectors.update(musan_speech_only_dvectors)
    ...
    config = {
        ...
        'musan_root': MUSAN_ROOT if (INCLUDE_NOISE_IDENTITY or INCLUDE_MUSAN_SPEECH_ONLY_IDENTITY) else '',
        ...
        'include_musan_speech_only_identity': INCLUDE_MUSAN_SPEECH_ONLY_IDENTITY,
        'n_musan_speech_only_samples': N_MUSAN_SPEECH_ONLY_SAMPLES if INCLUDE_MUSAN_SPEECH_ONLY_IDENTITY else 0,
        'musan_speech_only_sample_duration': MUSAN_SPEECH_ONLY_SAMPLE_DURATION if INCLUDE_MUSAN_SPEECH_ONLY_IDENTITY else 0,
        'sample_rate': SAMPLE_RATE,
        # Dataset stats
        ...
        'test_mixed_loss': eval_results['mixed_loss'] if eval_results is not None else None,
        'test_mse_loss': eval_results['mse_loss'] if eval_results is not None else None,
        'test_cosine_loss': eval_results['cosine_loss'] if eval_results is not None else None,
    ...
    if INCLUDE_MUSAN_SPEECH_ONLY_IDENTITY:
        print(f"  - MUSAN speech-only noise d-vectors: input -> same input")
